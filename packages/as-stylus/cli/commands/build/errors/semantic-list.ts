...
const SEMANTIC_ERRORS: Record<string, SemanticError> = {
  ...
  [ERROR_CODES.MULTIPLE_VISIBILITY_DECORATORS_FOUND]: {
    ...
    message: ([methodName]: string[]) =>
      `Method "${methodName}" cannot have both @External and @Public decorators`,
  ...
  [ERROR_CODES.MULTIPLE_STATE_MUTABILITY_DECORATORS_FOUND]: {
    ...
    message: ([methodName]: string[]) =>
      `Method "${methodName}" has multiple decorators of the same type`,
  ...
  [ERROR_CODES.INVALID_RETURN_TYPE]: {
    ...
    message: ([methodName]: string[]) =>
      `@View method "${methodName}" must return a serializable value`,
  ...
  [ERROR_CODES.MULTIPLE_FALLBACK_RECEIVE_DECORATORS]: {
    ...
    message: ([decorators]: string[]) =>
      `Method cannot have multiple fallback/receive decorators: ${decorators}`,
  ...
  [ERROR_CODES.FALLBACK_RECEIVE_CANNOT_BE_VIEW_PURE]: {
    ...
    description:
      "Fallback and receive functions cannot be marked as view or pure since they can modify state.",
