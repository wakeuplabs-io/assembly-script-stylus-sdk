...
import { contractService, ContractService } from "./contract-service.js";
import env from "../config/env.js";
import { getAbi, parseDeploymentOutput } from "./utils.js";
...
export async function setupE2EContract(
  ...
  run(`npx @wakeuplabs/as-stylus compile ${fileName} --endpoint ${env.RPC_URL}`, contractPath);

  const abi = getAbi(abiPath);

  const contractName = fileName.replace(/\.ts$/, "");
  const deployLog = deployContract(contractPath, env.PRIVATE_KEY, env.RPC_URL, contractName);
  const contractAddr = parseDeploymentOutput(deployLog);

  console.log("📍 Contract deployed at:", contractAddr);

  // Create contract service
  const contract = contractService(contractAddr.address as Address, abi, false);

  const constructor = abi.find(
    (method: { name: string }) => method.name === _options.constructorName,
  );
  ...
  if (constructor) {
    ...
    await contract.write(_options.walletClient!, constructor.name, constructorArgs);
...
export function fundUser(userAddress: string): void {
  run(
    `cast send ${userAddress} --value 0.1ether --private-key ${env.PRIVATE_KEY} --rpc-url ${env.RPC_URL}`,
  );
}
...
export function getBalance(address: string): bigint {
  const balanceOutput = run(`cast balance ${address} --rpc-url ${env.RPC_URL}`);
