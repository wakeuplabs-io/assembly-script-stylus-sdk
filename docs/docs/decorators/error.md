...
```typescript
// Error: "Error must be a class"
@Error
const InvalidError = { };  // Not a class

// Error: "Unsupported error field type"
@Error
class BadError {
  invalidField: ComplexObject;  // Unsupported type
}

// Error: "Error revert outside contract"
InsufficientBalance.revert(amount, balance);  // Must be inside contract method
```

## Best Practices

1. **Descriptive Names**: Use clear, descriptive error names
2. **Relevant Data**: Include all relevant data for debugging
3. **Consistent Structure**: Keep related errors structurally similar
4. **Error Hierarchy**: Group related errors together
5. **Documentation**: Document when and why errors are thrown
...
@Error
class TransferToZeroAddress {
  from: Address;
  amount: U256;
}

@Error
class ExceedsAllowance {
  owner: Address;
  spender: Address;
  requested: U256;
  allowance: U256;
...
@Contract
export class WellStructuredToken {
  @External
  static transfer(to: Address, amount: U256): void {
    const sender = msg.sender();
    
    // Clear error conditions with appropriate errors
    if (Address.isZero(to)) {
      TransferToZeroAddress.revert(sender, amount);
    }
    
    const balance = WellStructuredToken.balances.get(sender);
    if (balance.lessThan(amount)) {
      InsufficientBalance.revert(sender, amount, balance);
    }
...
- [`@External`](external.md) - Methods that can throw errors
- [`@Event`](event.md) - For logging events
- [`@View`](view.md) - Read-only methods (can also throw errors)

Custom errors provide a powerful and gas-efficient way to handle exceptional conditions in your smart contracts! 